...
- (void)viewDidLoad
{
    ...
    UILabel * labIntroudction= [[UILabel alloc] initWithFrame:CGRectMake(365, 140, 290, 50)];
    ...
    UIImageView * imageView = [[UIImageView alloc]initWithFrame:CGRectMake(360, 200, 300, 300)];
    imageView.image = [UIImage imageNamed:@"pick_bg"];
    [self.view addSubview:imageView];
    
    upOrdown = NO;
    num =0;
    _line = [[UIImageView alloc] initWithFrame:CGRectMake(400, 210, 220, 2)];
...
-(void)animation1
{
    if (upOrdown == NO) {
        num ++;
        _line.frame = CGRectMake(400, 210+2*num, 220, 2);
        if (2*num == 280) {
            upOrdown = YES;
        }
    }
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    ...
    [self dismissViewControllerAnimated:YES completion:^
     {
         [timer invalidate];
         [self setmyText:stringValue];
         NSLog(@"stringValue=%@",stringValue);
     }];
}

-(void) setmyText:(NSString*)stringValue{
        [self.delegate setDeskTextField:stringValue];
}
